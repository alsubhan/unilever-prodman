...
router.get('/', authMiddleware, async (req, res) => {
  ...
  const items = await db.all('SELECT * FROM raw_materials ORDER BY part_number', );
  res.json(items);
});
...
router.post('/', authMiddleware, requireRole('admin', 'production_manager'), async (req, res) => {
  ...
  try {
    const result = await db.run('INSERT INTO raw_materials (part_number, name, description, unit) VALUES (?, ?, ?, ?)', part_number, name, description || '', unit || 'kg');
...
router.get('/:id', authMiddleware, async (req, res) => {
  const db = await getDb();
  const item = await db.get('SELECT * FROM raw_materials WHERE id = ?', req.params.id);
...
router.put('/:id', authMiddleware, requireRole('admin', 'production_manager'), async (req, res) => {
  ...
  if (!item) return res.status(404).json({ error: 'Not found' });
  try {
    await db.run(`UPDATE raw_materials SET part_number=?, name=?, description=?, unit=?, is_active=?, updated_at=datetime('now') WHERE id=?`,
